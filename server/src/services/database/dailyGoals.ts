...
export class EnhancedDailyGoalsService {
    ...
    try {
      ...
      for (const user of users) {
        try {
          ...
          console.log(
            `📋 Questionnaire found for ${user.user_id}: ${!!questionnaire}`
          );
          ...
          if (verification) {
            console.log(
              `✅ VERIFIED: Goal exists in database with ID ${verification.id}`
            );
          } else {
            console.error(
              `❌ VERIFICATION FAILED: Goal not found in database!`
            );
      ...
      allTodayGoals.forEach((goal, index) => {
        console.log(
          `  ${index + 1}. ID: ${goal.id}, User: ${goal.user_id}, Calories: ${
            goal.calories
          }, Created: ${goal.created_at.toISOString()}, Updated: ${goal.updated_at.toISOString()}`
        );
      });
  ...
  static async forceCreateGoalsForAllUsers(): Promise<DailyGoalCreationResult> {
    ...
    try {
      ...
      for (let i = 0; i < allUsers.length; i++) {
        ...
        try {
          console.log(
            `\n🔄 [${i + 1}/${allUsers.length}] FORCE processing user: ${
              user.user_id
            } (${user.email})`
          );
   ...
   */
  static async forceCreateDailyGoalsForUser(
    userId: string
    ...
    try {
      ...
      console.error(
        `💥 ERROR force creating daily goals for user ${userId}:`,
        error
      );
      throw error;
    }
   ...
   */
  private static calculatePersonalizedGoals(
    questionnaire: any
  ): NutritionGoals {
    ...
    if (questionnaire) {
      ...
      const isMale =
        questionnaire.gender?.toLowerCase().includes("male") ||
      ...
      console.log(
        `🔢 BMR calculated: ${Math.round(bmr)} for ${
          isMale ? "male" : "female"
        }`
      );
      ...
      console.log(
        `⚡ TDEE calculated: ${Math.round(
          tdee
        )} (activity: ${activityLevel}, multiplier: ${activityMultiplier})`
      );
      ...
      switch (mainGoal) {
        ...
        case "WEIGHT_GAIN":
          ...
          console.log(
            `📈 Weight gain: ${Math.round(tdee)} + 300 = ${calories}`
          );
          break;
        case "SPORTS_PERFORMANCE":
          calories = Math.round(tdee + 200); // Slight surplus for performance
          console.log(
            `🏃 Sports performance: ${Math.round(tdee)} + 200 = ${calories}`
          );
          break;
        default:
      ...
      fats_g = Math.round((calories * 0.3) / 9); // 30% of calories from fats
